<template>
  ...
  >
    <div v-for="subject in subjectsFiltered" :key="subject.id">
      ...
      <div :id="subject.id + 'no_edit_mode'" class=" block m-3 px-3 py-1 bg-white bg-opacity-10 backdrop-filter backdrop-blur-xl shadow-2xl rounded-full" v-if="subject.state == 1">
        <span class=" bg-transparent outline-none" type="text" >{{subject.name}} </span>
        <i :id="subject.id + 'btn_edit'" @click="editSubjectInput(subject.id)" :class="'fa-pencil-alt text-yellow-300 hover:text-yellow-500 ' + icon_class"></i>
        <i :id="subject.id + 'btn_delete'" @click="removeSubject(subject)" :class="'fa-trash-alt text-red-400 hover:text-red-500 ' + icon_class"></i>
      </div>
        ...
        <div class="text-center pt-3 pb-1">
          ...
          >
            <i
              ...
              :class="'fa-save text-green-300 ' + icon_class"
            ></i>
            Guardar
          </span>
          <span
          ...
          >
            <i
              ...
              :class="'fa-times text-red-400 ' + icon_class"
            ></i>
            Cancelar
          </span>
        </div>
...
export default {
  name: "SubjectCardComponent",
  data: function() {
    return {
      icon_class: " fas mx-1 text-md drop-shadow-lg",
    };
  },
  computed: {
    ...mapState(["subjects"]),
